...
try:
    ...
    st.warning("Modèles SpaCy non trouvés. Installation requise : python -m spacy download en_core_web_lg && python -m spacy download fr_core_news_sm")
    st.stop()
...
def load_assets():
    ...
    try:
        response = requests.get("https://images.unsplash.com/photo-1620712943543-bcc4688e7485?q=80&w=3000")
...
def detect_language(text: str) -> str:
    """Détection robuste de la langue avec gestion des erreurs"""
    try:
        if len(text) < 20:
            return 'fr'  # Par défaut français pour les textes courts
            
        # Vérification manuelle des mots courants français
        french_words = ['le', 'la', 'les', 'un', 'une', 'des', 'et', 'est', 'dans']
        words = text.lower().split()
        french_count = sum(1 for word in words if word in french_words)
        
        if french_count / len(words) > 0.2:  # Si plus de 20% de mots français
            return 'fr'
        ...
        return 'fr'  # Retour par défaut français en cas d'erreur
    except Exception:
        return 'fr'
...
def hybrid_search(query: str, dataset: List[str], top_k: int = 10) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        st.info(f"Langue détectée: {'Français' if query_lang == 'fr' else 'Anglais'}")
        ...
        # Filtrer les résultats avec un score minimum de 0.4
        filtered_results = [res for res in unique_results.values() if res["combined_score"] >= 0.4]
        
        return sorted(filtered_results, key=lambda x: x["combined_score"], reverse=True)[:top_k]
...
def analyze_writing_style(text: str, language: str) -> Dict[str, Any]:
    ...
    try:
        ...
        metrics = {
            ...
            "sentiment": sum([sent.sentiment for sent in doc.sents]) / len(list(doc.sents)) if len(list(doc.sents)) > 0 else 0,
            "entities": [(ent.text, ent.label_) for ent in doc.ents],
            "avg_word_length": np.mean([len(token.text) for token in doc if not token.is_punct]) if len([token.text for token in doc if not token.is_punct]) > 0 else 0,
            ...
            "lexical_diversity": len(set([token.text.lower() for token in doc if not token.is_punct])) / 
                                len([token.text for token in doc if not token.is_punct]) if len([token.text for token in doc if not token.is_punct]) > 0 else 0,
            "avg_sentence_length": np.mean([len(sent) for sent in doc.sents]) if len(list(doc.sents)) > 0 else 0
        }
        
        return metrics
    except Exception as e:
        st.warning(f"Erreur dans l'analyse stylistique: {str(e)}")
        return {
            "readability": 0,
            "complexity": 0,
            "sentiment": 0,
...
def create_similarity_network(matches: List[Dict[str, Any]]) -> str:
    ...
    try:
        ...
        st.error(f"Erreur lors de la création du réseau: {str(e)}")
        return ""
...
def generate_wordcloud(matches: List[Dict[str, Any]]):
    ...
    try:
        ...
        st.warning(f"Erreur lors de la génération du nuage de mots: {str(e)}")
            ...
            if uploaded_file:
                try:
                    if uploaded_file.type == "application/pdf":
                        # Extraction texte PDF avec gestion d'erreurs
                        pdf_reader = PyPDF2.PdfReader(uploaded_file)
                        input_text = ""
                        for page in pdf_reader.pages:
                            try:
                                page_text = page.extract_text()
                                if page_text:  # Vérifie si l'extraction a retourné du texte
                                    input_text += page_text + "\n"
                            except Exception as e:
                                st.warning(f"Erreur lors de l'extraction d'une page PDF: {str(e)}")
                                continue
                    ...
                    else:  # Fichier texte simple
                        try:
                            input_text = uploaded_file.getvalue().decode("utf-8")
                        except UnicodeDecodeError:
                            st.warning("Encodage non-UTF-8 détecté, tentative avec latin-1...")
                            input_text = uploaded_file.getvalue().decode("latin-1")
                        
                    if not input_text.strip():
                        st.error("Aucun texte n'a pu être extrait de ce fichier.")
                        
                except Exception as e:
                    st.error(f"Erreur critique lors du traitement du fichier: {str(e)}")
                    input_text = ""
        else:
